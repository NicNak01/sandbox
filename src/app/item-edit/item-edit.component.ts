  ...
  displayMessage: { [key: string]: string } = {};
    ...
    if (this.item._id === 'none') {
      ...
      if (confirm(`Really delete the item: ${this.item.ItemName}?`)) {
        ...
        this.itemEditService.deleteitem(this.item._id)
          .subscribe(
            () => this.onSaveComplete(),
  ...
  saveItem(): void {
    ...
    if (this.itemForm.valid) {
      if (this.itemForm.dirty) {
        ...
        if (this.item._id === 'none') {
          this.itemEditService.createitem(p)
            .subscribe(
              () =>  this.onSaveComplete(),
              (error: any) => {this.errorMessage = <any>error; this.processing = false; }
            );
        } else {
          this.itemEditService.updateitem(p)
            .subscribe(
              () => this.onSaveComplete(),
              (error: any) => {this.errorMessage = <any>error; this.processing = false; }
            );
